...
class StrategyEngine:
    ...
    async def set_manual_rule(self, rule: str):
        ...
        await self.bus.emit(WoiEvent("STRATEGY_MANUAL_SET", "Manual strategy rule set", {"rule": rule[:1200]}))
    ...
    async def clear_manual_rule(self):
        ...
        self.state.manual_failures = 0
        await self.rules.clear_active(ts_utc=utc_now())
        await self.bus.emit(WoiEvent("STRATEGY_MANUAL_CLEAR", "Manual strategy cleared; reverting to auto", {}))

    async def record_manual_failure(self, reason: str):
        if self.state.mode != "manual_overlay":
            return
        self.state.manual_failures += 1
        await self.bus.emit(WoiEvent("STRATEGY_MANUAL_FAILURE", "Manual strategy failure", {"count": self.state.manual_failures, "reason": reason}, level="WARN"))

        # v1 auto-revert policy (tunable):
        if self.state.manual_failures >= 3:
            await self.bus.emit(WoiEvent(
    ...
    async def _loop(self):
        ...
        while self.state.running:
            tick += 1
            try:
                # Manual watchlist influences what WOI focuses on (modules can read it)
                wl = await self.watchlist.list()
                if tick % 10 == 0:
                    await self.bus.emit(WoiEvent("WATCHLIST_HEARTBEAT", "Current watchlist snapshot", {"count": len(wl), "symbols": [x["symbol"] for x in wl[:25]]}))

                # Polymarket scanning tick (if enabled)
                if self.cfg.polymarket_enabled:
                    await self.modules.polymarket.tick(tick=tick, strategy_state=self.state)

                # Scraper tick (if enabled)
                if self.cfg.scraper_enabled:
                    await self.modules.scraper.tick(tick=tick)

                if tick % 30 == 0:
                    await self.bus.emit(WoiEvent("HEARTBEAT", "WOI heartbeat", {"tick": tick, "mode": self.state.mode}))
            except Exception as e:
                await self.bus.emit(WoiEvent("STRATEGY_LOOP_ERROR", str(e), {"tick": tick}, level="ERROR"))
